    ...
    public class CreateBreakdownConsumer : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            consumer.ReceivedAsync += async (_, ea) =>
            {
                ...
                try
                {
                    var createBreakdownDto = JsonSerializer.Deserialize<CreateBreakdownDto>(message, new JsonSerializerOptions
                    {
                        PropertyNamingPolicy = JsonNamingPolicy.CamelCase
                    })!;
                    using var scope = _serviceProvider.CreateScope();
                    var ordersService = scope.ServiceProvider.GetRequiredService<IBreakdownsService>();

                    await ordersService.Create(createBreakdownDto);
